...
#pp[,1] <- paste0(rep(c(1,2,3),each=4),pp[,1])
#anno <- mlt_1[,1:2]
#annotation <- (anno %>% separate(Stimulation,c("DMSO","Stimulation")))[,c(1:3)]
t <- t(pp_n) > 1.30103
t[t=="TRUE"] <- "*"
t[t=="FALSE"] <- " "
#t[8,4] <- t[5,4] <- t[3,1] <- "**"
#t[5,4] <- t[3,4] <- t[2,1] <- "**"
#t[1,8] <- t[4,3] <- t[6,3] <- t[6,6] <- "***"
f1 = colorRamp2(seq(min(mlt1_n), max(mlt1_n), length = 3), c("blue", "#EEEEEE", "red"))
...
quartz()
aheatmap(t(mlt1_n),annCol=list(Stimulation=annotation$Contrast,Visit=annotation$Visit),Colv=NA,
         Rowv= NA,cellwidth = 16, cellheight = 18, fontsize = 8,labCol = NA,color = "-RdYlBu2:100",
         breaks=NA,main = "Mean foldchange in comparison to DMSO",txt=t)
#breaks=seq(-0.001,0.0005,0.00005)
text(x=0.84,y=0.26,"             *Decrease significantly
        **Increase significantly
                                  ***Mean Foldchange between 0.9 and 1.1
              Mixed effect regression,
                            BH-adjusted p-value cutoff = 0.05",cex=0.6)
quartz()
aheatmap(t(mlt1_n),annCol=list(Stimulation=annotation$Stimulation,Visit=annotation$Visit),Colv=NA,
         Rowv= NA,cellwidth = 16, cellheight = 18, fontsize = 8,labCol = NA,color = "-RdYlBu2",
         breaks=seq(0,5,0.1),main = "Foldchange in comparison to DMSO",txt=t)
text(x=0.9,y=0.3,"a",cex=0.5)

write.xlsx(fc_mean[c(2,3,5,7,10,12),],"/Users/amandava/Documents/Rochester_AsPIRES/AsPIRES_data/fc_mean.xlsx",row.names=TRUE)
